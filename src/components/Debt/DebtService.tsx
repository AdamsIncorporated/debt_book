...
interface DebtServiceFormProps {
    onNext: () => void;
    onBack: () => void;
    onSubmitData?: (rows: DebtServiceRow[]) => void; // optional for backend
}
...
const DebtServiceForm: React.FC<DebtServiceFormProps> = ({ onNext, onBack, onSubmitData }) => {
    ...
    const handleDownloadTemplate = () => {
        const worksheet = XLSX.utils.json_to_sheet([
            { periodEndDate: "YYYY-MM-DD", principal: 0, coupon: 0, interest: 0, debtService: 0 },
        ]);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "DebtServiceTemplate");
